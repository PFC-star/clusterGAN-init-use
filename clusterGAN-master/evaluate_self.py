...
try:
    ...
    from clusgan.models_SC import Generator_CNN, Encoder_CNN, Discriminator_CNN
...
def evaluate():
    ...
    parser.add_argument("-r", "--run_dir", dest="run_dir",
                        default="mnist\\100epoch_z30_wass_bs128_clusganSC_betac25_reference_ssh",
    ...
    # Latent space info
    train_df = pd.read_csv('%s\\training_details.csv' % (run_dir))
    ...
    # Load encoder model
    # encoder = Encoder_CNN(latent_dim, n_c, x_shape=x_shape).to(device)
    ...
    for data in dataloader:
        ...
        output_enc_zn,   enc_zc, ege_zc_logits = encoder(c_imgs)
        # 预测结果
        ...
        """
        使用 true_img--> encoder == zn + zc  --> generator = gen_img --> encoder ==  zn + zc 效果会如何呢？
        """
        # zn + zc  --> generator = gen_img
        gen_img = generator(output_enc_zn, enc_zc)
        # gen_img --> encoder ==  zn + zc
        # output_ege_zn, ege_zn, ege_zn_H, ege_zn_B, ege_zc, ege_zc_logits = encoder(gen_img)
        output_ege_zn,  ege_zc, ege_zc_logits = encoder(gen_img)
        pred_ege = torch.max(ege_zc, 1)[1]
        # 指标
        nmi_ege += NMI(pred_ege.cpu(), labels.cpu())
        ari_ege += ARI(pred_ege.cpu(), labels.cpu())
        acc_ege += ACC(pred_ege.cpu(), labels.cpu())

        # zc_loss = xe_loss(enc_zc_logits, labels)
        # eval_loss += zc_loss.data

        # real_pred = pred2realpred(pred)

        # Convert to numpy for indexing purposes
        # labels = labels.cpu().data.numpy()
        # num_correct = (real_pred == labels).sum().item()
    ...
    print('NMI_ege:{:.6f}'.format(nmi_ege / len(dataloader)))
    print('ARI_ege:{:.6f}'.format(ari_ege / len(dataloader)))
    print('ACC_ege:{:.6f}'.format(acc_ege / len(dataloader)))
